...
(function(undefined) {
    ...
    /**
     * @description: sum数组求和主要注意的是遇到是字符串的数字,以及不是数字的--等,统一计算为0
     * @author: kelichao
     * @update: 
     * @length :保留的位数长度.不然的话会出现精度问题
     */
    kit.sum = function(arr, length) {
        ...
        return Number(sum.toFixed(length || 0));
    }
     ...
     * @arr : 数组
     * @length : 截取位数
     * @flag : 是否要将空值"--",这类数值计算入总数
     * @demo : kit.average([1,2,3,"--"], 2, true)
     */
